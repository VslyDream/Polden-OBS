...
Для переносимого архива запустите `python scripts/package-windows.py`: он копирует готовый каталог Release в ZIP без отладочных `.pdb`, локального профиля `config/` и персонального плагина InfoWriter. Архив включает все остальные рабочие библиотеки, ресурсы, плагины и `portable_mode.txt`. Вариант `--include-local-settings` создаёт отдельный персональный ZIP только с выбранным профилем записи, коллекцией сцен, `user.ini` и InfoWriter, исключая журналы, cookies и другие файлы конфигурации. Такой архив нельзя публиковать без проверки личных данных.

При запущенной основной сборке используйте отдельный каталог: `python scripts/build-windows.py --build-dir build_polden_next`, затем `python scripts/package-windows.py --build-dir build_polden_next --include-local-settings`. Персональные настройки в этом случае берутся из прежней портативной версии `build_x64/rundir/Release/`; новый EXE и библиотеки — из `build_polden_next`. Оба ZIP включают панель для Premiere в `premiere/Polden-OBS-Bridge-premierepro.ccx` и её исходники в `premiere/polden-bridge`; инструкции по установке находятся в [premiere.md](premiere.md). Отдельный `.ccx` создаётся командой `python scripts/package-premiere.py`. Текущая отдельная сборка прошла и `obs64.exe --version` вернул `Polden OBS - 32.2.2-5-gb790522-modified`. Реальная запись и импорт в Premiere ещё требуют проверки в этих приложениях.
